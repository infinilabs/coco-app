...
//         })?;
//
//     let datasources = parse_search_results(resp).await.map_err(|e| {
//         dbg!("Error parsing search results: {}", &e);
//         e.to_string()
//     })?;
//
//
//     let connectors=get_connectors_from_cache_or_remote(id.as_str()).await?;
//
//     let datasources: Vec<DataSource> = datasources
//         .into_iter()
//         .map(|mut datasource| {
//             if let Some(existing_connector) = connectors.get(&datasource.id) {
//                 // If found in cache, update the connector's info
//                 // dbg!("Found connector in cache for {}: {:?}", &datasource.id, &existing_connector);
//                 datasource.connector_info = Some(existing_connector.clone());
//             }
...
//
//         // Attempt to get datasources by server, and continue even if it fails
//         let connectors = match get_datasources_by_server(app_handle.clone(), server.id.clone()).await {
//             Ok(connectors) => {
//                 // dbg!("Found dataSources for server: {}, {:?}", &server.id,&connectors);
//
//                 let connectors_map: HashMap<String, DataSource> = connectors
//                     .into_iter()
//                     .map(|mut connector| {
//                         if let Some(existing_connector) = get_connector_by_id(&server.id, &connector.id) {
//                             // If found in cache, update the connector's info
//                             // dbg!("Found connector in cache for {}: {:?}", &connector.id, &existing_connector);
//                             connector.connector_info = Some(existing_connector);
//                             // Return the tuple to insert into the HashMap
//                             (connector.id.clone(), connector)
//                         } else {
//                             // Return the tuple to insert into the HashMap
//                             (connector.id.clone(), connector)
...
//         };
//
//         dbg!("DataSources for server:{}, {:?}", &server.id,&connectors);
//         {
//             let mut cache = DATASOURCE_CACHE.write().unwrap();
//             cache.insert(server.id.clone(), connectors);
//         }
//     }
//
//     // Now after the loop, perform a read operation on the cache
//     let cache_size = {
//         let cache = DATASOURCE_CACHE.read().unwrap();
//         cache.len()
//     };
//     dbg!("datasource_map size: {:?}", cache_size);
//
...
//     dbg!("get_datasources_by_server: id =", &id);
//
//     let cache=get_datasources_from_cache(&id);
//     if let Some(cache) = cache {
//         let datasources: Vec<DataSource> = cache.values().cloned().collect();
//         dbg!("Cached datasources: {:?}", &datasources);
//         return Ok(datasources);
//     }else {
//         dbg!("No cached datasources found for server: {}", &id);
//     }
//
//
